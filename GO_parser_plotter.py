...
parser = argparse.ArgumentParser(usage='python GO_parser_plotter.py -a input_file1 -i input_file1name -b input_file2 -j input_file2name -p pvalue_threshold -o output_file_name', add_help=True,formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    for k in df2['Description']:
        if i == k:
            ...
            if theNumToCompare < float(args.pvalue) and theOtherNumToCompare < float(args.pvalue):
                nums_dict[str(i)] = [whatIwant['Over_represented_pValue'].item(), theOtherThing['Over_represented_pValue'].item()]
...
ax = sns.heatmap(numbers, yticklabels = nums_dict.keys(), xticklabels = [args.input1name, args.input2name], vmax = args.pvalue, vmin = 0, cmap="plasma")
